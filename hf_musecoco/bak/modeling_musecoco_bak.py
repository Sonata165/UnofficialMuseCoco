...
class MuseCocoBlock(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        attention_class = MUSECOCO_ATTENTION_CLASSES[config._attn_implementation]
        ...
        self.mlp = MuseCocoMLP(inner_dim, config)
    ...
    def forward(
        ...
        attn_output = attn_outputs[0]  # output_attn: attention_out, present, (attentions)
...
class MuseCocoModel(MuseCocoPreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([MuseCocoBlock(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
@add_start_docstrings(
    """
    The MUSECOCO Model transformer with a language modeling head on top (linear layer with weights tied to the input
    embeddings).
    """,
    MUSECOCO_START_DOCSTRING,
)
class MuseCocoLMHeadModel(MuseCocoPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.transformer = MuseCocoModel(config)
...
@add_start_docstrings(
    """
The MuseCoco Model transformer with a language modeling and a multiple-choice classification head on top e.g. for
...
class MuseCocoDoubleHeadsModel(MuseCocoPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]
    ...
    def parallelize(self, device_map=None):
        warnings.warn(
            "`MuseCocoDoubleHeadsModel.parallelize` is deprecated and will be removed in v5 of Transformers, you should"
    ...
    @add_start_docstrings_to_model_forward(MUSECOCO_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=MuseCocoDoubleHeadsModelOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        past_key_values: Optional[Tuple[Tuple[torch.Tensor]]] = None,
        ...
        return_dict: Optional[bool] = None,
        **kwargs,
    ) -> Union[Tuple, MuseCocoDoubleHeadsModelOutput]:
        r"""
        ...
        ```python
        >>> import torch
        >>> from transformers import AutoTokenizer, MuseCocoDoubleHeadsModel

        >>> tokenizer = AutoTokenizer.from_pretrained("muzic/musecoco-large")
        >>> model = MuseCocoDoubleHeadsModel.from_pretrained("muzic/musecoco-large")
        ...
        return MuseCocoDoubleHeadsModelOutput(
            loss=lm_loss,
            mc_loss=mc_loss,
            logits=lm_logits,
            mc_logits=mc_logits,
