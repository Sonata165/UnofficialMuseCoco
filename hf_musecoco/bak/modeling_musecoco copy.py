...
class MuseCocoBlock(nn.Module):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        # utils.get_activation_fn(
        #     activation=str(config.activation_fn)
        #     if getattr(config, "activation_fn", None) is not None
        #     else "relu"
        # )
        ...
        # else:
        #     self.encoder_attn = self.build_encoder_attention(self.embed_dim, config)
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask=None, key_padding_mask=None):
        ...
        new_values = self.inner_attention(    # Produce nan in fp16   19:37 nan earlier than here
            queries,
            keys,
            values,
            attn_mask,
        ...
        ret = self.out_proj(new_values) # Project the output and return
